...
def read_headers(test_case_name,sheetname):
    wb = open_workbook(r"C:\Users\heera\PycharmProjects\Bharthi Axa_Project\TestData\inputs.xls")
...
def read_data(test_case_name, sheetname):
    ...
    for i in range(0, used_rows):
        rows = ws.row_values(i)
        if rows[0] == test_case_name:
            temp_data = [ item.strip() for item in rows if isinstance(item, str) and item.strip()]
